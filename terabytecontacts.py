...
class AddContact(webapp.RequestHandler):
	def post(self):
		...
		p1 = datamodel.Contact()
		p1.firstName = cgi.escape(self.request.get('p1firstName'))
		p1.lastName = cgi.escape(self.request.get('p1lastName'))
		p1.email = cgi.escape(self.request.get('p1email'))
		p1.cellPhone = cgi.escape(self.request.get('p1cellPhone'))
		p1.homePhone = cgi.escape(self.request.get('homePhone'))
		p1.contactType = "PARENT"
		p1.relations.append(contact.key())
		p1.put()
		
		p2 = datamodel.Contact()
		p2.firstName = cgi.escape(self.request.get('p2firstName'))
		p2.lastName = cgi.escape(self.request.get('p2lastName'))
		p2.email = cgi.escape(self.request.get('p2email'))
		p2.cellPhone = cgi.escape(self.request.get('p2cellPhone'))
		p2.homePhone = cgi.escape(self.request.get('homePhone'))
		p2.contactType = "PARENT"
		p2.relations.append(contact.key())
		p2.relations.append(p1.key())
		p2.put()
		
		p1.relations.append(p2.key())
		p1.put()
		
		contact.relations.append(p1.key())
		contact.relations.append(p2.key())
		contact.put()
